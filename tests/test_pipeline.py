...
def test_run_pipeline_errors_when_filters_remove_all_data(tmp_path, monkeypatch) -> None:
    """Running with filters that exclude every row should raise an error."""
    ...
    monkeypatch.setattr(
        ...
        replace(
            config.pipeline_config,
            raw_data_path=raw_path,
            filters=replace(config.pipeline_config.filters, start_period="2025-01-01"),
        ),
    )
    ...
    try:
        ...
        assert "No rows available" in str(exc)
    else:
        raise AssertionError("Pipeline should have raised an error when filters excluded all rows.")
